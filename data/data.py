...
class PrescriptionPillData(Dataset):
    def __init__(self, json_files, mode, args):
        ...
        self.all_pill_labels = get_all_pill_label(args.data_folder)
    ...
    def create_graph(self, bboxes, imgw, imgh, pills_class):
        ...
        for src_idx, src_row in enumerate(bboxes):
            ...
            if src_row['label'] == 'drugname':
                mapping_str = str(src_row['mapping'])
                if mapping_str not in pills_class:
                    pills_label = torch.tensor(-1, dtype=torch.long)
                else:
                    pills_label = torch.tensor(
                        self.all_pill_labels[mapping_str], dtype=torch.long)
            else:
                pills_label = torch.tensor(-1, dtype=torch.long)
    ...
    def __getitem__(self, idx):
        ...
        # FOR IMAGE PILLS
        pills_image_folder_name = osp.splitext(osp.basename(self.json_files[idx]))[0]
        pills_image_path = osp.join(self.args.data_folder, self.args.image_path, self.mode, pills_image_folder_name)
...
def get_all_pill_label(data_folder):
    classes = set()
    for mode in ["train", "test"]:
        path = osp.join(data_folder, "pills", mode)
        if osp.exists(path):
            for entry in os.scandir(path):
                if entry.is_dir():
                    for subentry in os.scandir(entry.path):
                        if subentry.is_dir():
                            classes.add(subentry.name)
